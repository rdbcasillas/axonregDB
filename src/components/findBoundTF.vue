    ...
    <b-container>
      <b-row>
        <b-col cols="10">
          ...
          <br />
          <b-row v-if="flag">
            <b-card-group deck v-for="(obj,index) in geneTFarr" :key="index">
              <b-card
              ...
              >
                <b-list-group
                  v-for="(item,index2) in obj.rnadata"
                  :key="index2"
                  class="mainpanel"
                  flush
                >
                  <b-list-group-item
                    v-if="parseInt(item[selected])>=slidervalue[0] 
                                && parseInt(item[selected])<=slidervalue[1]"
                    class="genebox"
                  >{{ item.external_gene_name }}</b-list-group-item>
                </b-list-group>
              </b-card>
            </b-card-group>
          </b-row>
          ...
          <b-card-group deck v-if="showcommon">
            <b-card class="sidepanelheader" header="Top common TFs">
              <b-list-group class="sidepanel" v-for="(tf,index) in impTF" :key="index">
                <b-list-group-item
                  :variant="tf.count == geneArray.length ? 'success' : 'warning'"
                  class="d-flex justify-content-between align-items-center"
                >
                  {{ tf.tf }}
                  <b-badge variant="primary" pill>{{ tf.count }}</b-badge>
                </b-list-group-item>
              </b-list-group>
            </b-card>
          </b-card-group>
...
export default {
  ...
  methods: {
    getCount(age, data) {
      ...
      let newarr = _.each(
        data,
        (item) => (item[age] = parseInt(item[age], 10))
      );
    ...
    loadTF() {
      ...
      let filtTFdata = _.compact(
        _(this.expdata).keyBy("external_gene_name").at(allTFarr[0]).value()
      );
    ...
    async loadRNAdata() {
      this.expdata = await d3.tsv("./datasets/targetGenes/TFrnaexp.tsv");
    },
  },
  created() {
    this.loadRNAdata();
