<template>
  <div>
    <b-container>
      ...
      <br />
      <b-row v-if="flag">
        <b-card-group deck v-for="(obj, index) in tfgenearr" :key="index">
          <b-card
          ...
          >
            ...
            >
              <b-list-group-item
                v-if="
                  parseInt(item[selected]) >= slidervalue[0] &&
...
export default {
  ...
  methods: {
    ...
    async getTFs() {
      let tflist = await d3.tsv(
        "https://raw.githubusercontent.com/rdbcasillas/axonregDB/master/public/datasets/targetGenes/TFlist.txt"
      );
    ...
    async loadGenes() {
      ...
      this.geneSet1 = _.remove(_.map(genelist, "genelist"), function(item) {
        return item != "";
      });
      for (let tf in this.tfArray) {
        let newgenelist = {};
        newgenelist["tfname"] = this.tfname;
        newgenelist["rnadata"] = _.compact(
          _(this.expdata)
